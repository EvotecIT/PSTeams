...
Provider operations whose acknowledgement is produced by the handler are an explicit exception: they must run inline on the original request, use bounded process-local replay protection, and are not persisted or replayed after a host restart. This currently includes Discord autocomplete, Slack modal view submissions, and Teams Adaptive Card invoke responses. Use provider retry behavior and idempotent handlers for those synchronous operations.
