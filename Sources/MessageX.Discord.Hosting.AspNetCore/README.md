...
Commands and components use deferred acknowledgements before asynchronous dispatch by default. Register a command or component handler with `MessageDispatchMode.Synchronous` when it returns `DiscordInteractionAcknowledgement.Modal(...)`; Discord requires that modal acknowledgement on the original request. Autocomplete selects inline dispatch automatically so its handler can return typed choices in the initial response; the empty choice list is only the fallback acknowledgement. Inline responses use bounded process-local replay protection and are explicitly outside durable persistence and restart replay. Durable follow-up responses require the still-pending secure interaction-capability owner; tokens are never stored in durable envelopes.
